...
class BranchyNet(nn.Module):
  ...
  def initialize_main_model(self):
    if (self.model_name == "AlexNet"):
      ...
      if not self.imageNet:
        self.model = set_parameter_requires_grad(self.model, self.feature_extraction)
        in_feature = self.model.classifier[6].in_features
        self.model.classifier[6] = nn.Linear(in_feature, self.n_classes)
  ...
  def countFlop(self, input_size):
    ...
    """
    input = torch.rand(1, input_size[0], input_size[1], input_size[2])
    ops, all_data = count_ops(self.model, input, print_readable=False, verbose=True)
    flop_idx_dict = {i: 0 for i in range(len(all_data))}
    flop_layer_dict = {}
  ...
  #  pred_list, conf_list = [], []
  #  for i, layer in enumerate(self.branchyNet):
  #    if (isinstance(layer, ClassifierModule)):
  #      pred = layer(x)
  ...
  #      cont_branch += 1
  #      pred = layer(x)
  #      infered_conf, infered_class = torch.max(softmax(pred), 1)
  #      conf_list.append(infered_conf)
  #      if (infered_conf.item() > p_tar):
  #        return pred, cont_branch 
  ...
  #  else:
  #    max_conf = np.argmax(conf_list)
  #    return pred_list[max_conf], cont_branch
